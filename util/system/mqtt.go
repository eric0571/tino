...
func EmqttClient(username string) MqttClient {
	mc := MqttClient{
		username: username,
		password: mqttPassword(config.Mqtt.SecretKey, username),
	}
...
func (m *MqttClient) connect() {
	...
	opts.AddBroker(fmt.Sprintf("tcp://%s:%d", config.Mqtt.Hostname, config.Mqtt.Port))
	opts.SetClientID(m.username)
	opts.SetUsername(m.username)
	opts.SetPassword(m.password)
	opts.SetCleanSession(false)
	opts.SetKeepAlive(time.Duration(config.Mqtt.KeepAlive) * time.Second)
	opts.SetPingTimeout(time.Duration(config.Mqtt.PingRespTimeout) * time.Second)
